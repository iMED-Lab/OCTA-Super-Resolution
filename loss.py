...
class AssociationLoss(nn.Module):
    def __init__(self, metric='cos', spagg=True, spagg_alpha=0.5, asso_topk=1,
        ...
        self.BCELoss = torch.nn.L1Loss(reduce=True).cuda()
        self.metric = metric
        self.spagg = spagg
        self.spagg_alpha = spagg_alpha
        self.asso_topk = asso_topk
        self.print_info = print_info
        # self.attention = NONLocalBlock2D(in_channels=64, inter_channels=64).cuda()
    ...
    def associate(self, sim_mat, topk=1):
        indices = torch.topk(sim_mat, dim=2, k=topk)[1]
        # print(indices[1])
        # indices = torch.topk(sim_mat, dim=2, k=topk).indices.detach()
        sim = torch.topk(sim_mat, dim=2, k=topk)[0]
        return indices, sim
    ...
    def forward(self, x1, x2,x3):
        ...
        # x3 =
        # gt1 = (x1>0.2).type(torch.cuda.FloatTensor).detach()
        # print(gt1.max())
        # gt1 = gt1.float()
        N, _, H, W = x1.size()
        _, _, H2, W2 = x2.size()
        assert(x1.shape[:2] == x2.shape[:2] == x3.shape[:2]), x2.size()
        ...
        sim_mat_13, sim_mat_31 = self.build_correlation(x1, x3, metric=self.metric)
        # print(sim_mat_12.max())
        sim, indices, covered_indices = self.cycle_associate(sim_mat_12, sim_mat_21, topk=self.asso_topk)
        sim1, indices1, covered_indices1 = self.cycle_associate(sim_mat_13, sim_mat_31, topk=self.asso_topk)


        # print(indices.size())
        sim = sim.view(N, H, W, -1)
        sim1 = sim1.view(N, H, W, -1)
        # print(sim.min())

        ass_gt = (sim>0.6).type(torch.cuda.FloatTensor).detach()
        # print(sim)
        sim1 = ass_gt*sim1
        # print(sim1)
        # print(sim.min())
        ...
        # else:
        loss['association'] = self.BCELoss(sim1, ass_gt)
...
# b = torch.rand((2,64,32,32)).cuda()
# a_ = torch.rand((2,1,32,32)).cuda()
# c = AssociationLoss()(a,b,a_)
# print(c)
