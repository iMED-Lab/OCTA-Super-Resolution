...
from resnet import D_net,D1_net
...
test_batch_size = 1
num_epochs = 200
file_dir = '/home/imed/personal/hhy/domain_adaption_model'
# CT dataset
file_path = '/media/imed/d62e02b2-178f-4b3e-9c3f-4515bf343635/hhy/OCTA_super_resolution/rose/optovue3x3_304'
file_path1 = '/media/imed/d62e02b2-178f-4b3e-9c3f-4515bf343635/hhy/OCTA_super_resolution/rose/optovue6x6_152'
file_path2 = '/media/imed/d62e02b2-178f-4b3e-9c3f-4515bf343635/hhy/OCTA_super_resolution/zeiss/test_6x6_ori'
file_path_1 = '/media/imed/d62e02b2-178f-4b3e-9c3f-4515bf343635/hhy/OCTA_super_resolution/zeiss/test_6_256'
#file_path_2 = '/media/imed/9bb6637f-ee14-4ce3-a368-215ff60d1391/py_hhy/super_resolution_dataset/dataset/train/ROSE-ALL/6X6_400_1'
file_path_3 = '/media/imed/d62e02b2-178f-4b3e-9c3f-4515bf343635/hhy/OCTA_super_resolution/rose/OCTA_500_test'
# file_path = '/media/imed/9bb6637f-ee14-4ce3-a368-215ff60d1391/py_hhy/super_resolution_dataset/dataset/train/optovue_zeiss/train_3'
# file_path1 = '/media/imed/9bb6637f-ee14-4ce3-a368-215ff60d1391/py_hhy/super_resolution_dataset/dataset/train/optovue_zeiss/train_6'
# file_path_3 = '/media/imed/9bb6637f-ee14-4ce3-a368-215ff60d1391/py_hhy/super_resolution_dataset/dataset/train/optovue_zeiss/test_6'

train_data_loader = DataLoader(dataset=DatasetBlindSR(root=file_path, root_1=file_path1, root_2=None, isTraining=True),
                                          batch_size=train_batch_size,
                                          shuffle=True)
test_data_loader = DataLoader(dataset=DatasetBlindSR(root=file_path, root_1=file_path1, root_2=file_path_3, isTraining=False),
...
if __name__ == '__main__':
    ...
    G_model_path = '/media/imed/d62e02b2-178f-4b3e-9c3f-4515bf343635/hhy/OCTA_super_resolution/domain_adaption_model/haohy_optovue_sr/G2-250.pth'
    ...
    G2.load_state_dict(model_dict1)
    ...
    # G1 = nn.DataParallel(G1, device_ids=[0,1])
    # G2 = nn.DataParallel(G2, device_ids=[0,1])
    # D = nn.DataParallel(D, device_ids=[0,1])
    # D1 = nn.DataParallel(D1).cuda()
    #G1 = nn.DataParallel(G1).to(device)
    # loss_vgg = lpips.LPIPS(net='vgg').cuda()
    # D1 = nn.DataParallel(D1).cuda()

    # Loss function
    criterion1 = torch.nn.L1Loss().to(device)
    criterion2 = torch.nn.BCELoss(reduce=True).to(device)
    criterion3 = torch.nn.BCELoss(reduce=True).to(device)
    criterion4 = torch.nn.MSELoss(reduce=True).to(device)
    criterion5 = torch.nn.MSELoss(reduce=True).to(device)
    ssim_loss = pytorch_ssim.SSIM(size_average=False).to(device)
    ssim_loss_ = pytorch_ssim.SSIM(size_average=True).to(device)
    mmd_loss  = MMD_loss().to(device)
    # wavelet_dec = WaveletTransform(dec=True).cuda()
    # Optimizers
    G2_optimizer = torch.optim.Adam(G2.parameters(), lr = learning_rate_G2, betas=betas)
    ...
    file_name = 'haohy_domain_ada_sr_optovue_ours_da'
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, images1, images2) in enumerate(train_data_loader):
            ...
            #     param.requires_grad = False
            # image data
            x_lr = images.to(device)
            x_hr = images1.to(device)
            x_lr_r = images2.to(device)
            # x_lr_r_ori = images3.to(device)
            image_size = x_lr.size()
            # Train discriminator with real data
            # D_loss
            x_hr_gen0 = G(x_lr)
            x_hr_gen = G1(x_lr)
            x_hr_gen_r = G2(x_lr_r)
            # x_hr_gen_r_ori = G2(x_lr_r_ori)
            ...
            mmd_score1 = Normalize((mmd_score))
            print(mmd_score1)
            ...
            # ori_wavelet = wavelet_dec(x_hr_gen)
            # ori_wavelet_lf = ori_wavelet[:, 1:, :, :]
            # gen_image_wavelet = wavelet_dec(x_hr_gen_r)
            # gen_image_wavelet_lf = gen_image_wavelet[:, 1:, :, :]
            ...
            D_real_loss = weighted_mse_loss(D_real_decision, y_real_, mmd_score1)
            D_fake_decision = D(x_hr_gen_r).squeeze()
            D_fake_loss = criterion5(D_fake_decision, y_fake_,)
            # D1_loss
            # ori_wavelet = wavelet_dec(x_hr_gen)
            # ori_wavelet_lf = ori_wavelet[:, 1:, :, :]
            # gen_image_wavelet = wavelet_dec(x_hr_gen_r_ori)
            # gen_image_wavelet_lf = gen_image_wavelet[:, 1:, :, :]
            # D1_real_decision = D1(ori_wavelet_lf).squeeze()
            # mini_batch1 = D1_real_decision.size()
            # y_real_1 = Variable(torch.ones(mini_batch1).to(device))
            # y_fake_1 = Variable(torch.zeros(mini_batch1).to(device))
            # D1_real_loss = criterion5(D1_real_decision, y_real_1)
            # D1_fake_decision = D1(gen_image_wavelet_lf).squeeze()
            # D1_fake_loss = criterion4(D1_fake_decision, y_fake_1)
            ...
            # D1_mean_loss += (D1_real_loss + D1_fake_loss).item()
            # vis.plot(name='D1_loss', y=D1_mean_loss / (i + 1))
            # G_loss
            # x_hr_gen = G1(x_lr)
            # x_hr_gen0 = G(x_lr)
            # x_hr_gen = G1(x_lr)
            ...
            # gen_image_wavelet_lf = gen_image_wavelet[:, 2:3, :, :]
            # ssim_score = ssim_loss(x_hr_gen0, x_hr[:, 0:1, :, :])
            ssim_loss1 = 1 - (ssim_loss(x_hr_gen, x_hr[:, 0:1, :, :])).mean(dim=0)
            ...
            # D1_fake_decision = D1(gen_image_wavelet_lf).squeeze()
            # D1_loss2 = criterion5(D1_fake_decision, y_real_1)
            mse_loss2 = criterion1(x_hr_gen_r, x_hr[:, 0:1, :, :])
            ssim_loss2 = 1 - ssim_loss_(x_hr_gen_r, x_hr[:, 0:1, :, :])
            mse_loss = criterion1(x_hr_gen, x_hr[:, 0:1, :, :])
            ...
            G_loss = 4 * ssim_loss1 + 8 * mse_loss + D_loss2 # Back propagation
            ...
            # x_lr_r_up_ori = torch.nn.Upsample(scale_factor=2)(x_lr_r_ori)
            out_vis = torch.cat(
                [x_lr_up[0:4, 0:1, :, :], x_hr[0:4, 0:1, :, :], x_hr_gen[0:4,:,:,:], x_lr_up_r[0:4,0:1,:,:], x_hr_gen_r[0:4,:,:,:]], 0)
        ...
        if epoch % 1 == 0:
            torch.save(G2, checkpoint_path.format(net='G2',epoch=epoch))
        avg_psnr = 0.0
        ...
        with torch.no_grad():
            ...
            for i, (images) in enumerate(test_data_loader):
                ...
                en_score = mass_metrics.main(pred_img)
                score = score + en_score
                if epoch % 1 == 0:
                    path = '/media/imed/d62e02b2-178f-4b3e-9c3f-4515bf343635/hhy/OCTA_super_resolution/result'
                    checkpoint_path = os.path.join(path, file_name,'{net}-{epoch}.pth'.format(net='G2',epoch=epoch))
                    if not os.path.exists(checkpoint_path):
                        os.makedirs(checkpoint_path)

                    cv2.imwrite(os.path.join(checkpoint_path, name), pred_img)

            vis.plot(name='entropy', y = score / len(test_data_loader))
